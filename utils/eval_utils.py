...
def initiate_model(args, ckpt_path=None):
    ...
    if args.model_size is not None and args.model_type in [
        "clam",
        "attention_mil",
        "clam_new",
    ]:
...
def summary(model, loader, args):
    ...
    if args.n_classes > 2:
        acc1, acc3 = accuracy(
            torch.from_numpy(all_probs), torch.from_numpy(all_labels), topk=(1, 3)
        )
...
def compute_features(dataset, args, ckpt_path, save_dir, model=None, feature_dim=512):
    ...
    initialize_features_hdf5_file(
        file_path, len(dataset), feature_dim=feature_dim, names=names
    )
...
def save_features(dataset, idx, model, args, save_file_path):
    ...
    with torch.no_grad():
        if type(model) == CLAM:
            _, Y_prob, Y_hat, _, results_dict = model(
                features, instance_eval=False, return_features=True
            )
...
def initialize_features_hdf5_file(file_path, length, feature_dim=512, names=None):
    ...
    if names is not None:
        ...
        label_dset = file.create_dataset(
            "names", shape=(length,), chunks=(1,), dtype=dt
        )

    label_dset = file.create_dataset(
        "label", shape=(length,), chunks=(1,), dtype=np.int32
    )

    pred_dset = file.create_dataset(
        "Y_hat", shape=(length,), chunks=(1,), dtype=np.int32
    )

    prob_dset = file.create_dataset(
        "Y_prob", shape=(length,), chunks=(1,), dtype=np.float32
    )
...
def eval2(datasets: tuple, cur: int, args: Namespace):
    ...
    if args.pretrain_VAE:
        ...
        ckpt = torch.load(
            "./VAE/logs/tcga_base/000-all/%d/%d/%d_best.ckpt" % (cur, cur, cur)
        )
        ...
        model_dict.update(
            {
                ...
                "tcga_proj": "_".join(args.task.split("_")[:2]),
                "split_idx": cur,
            }
        )
    ...
    if args.task_type == "survival":
        results_val_dict, val_c_index = summary_survival(
            model, val_loader, args.n_classes, VAE
        )
        print("Val c-index: {:.4f}".format(val_c_index))

        results_test_dict, test_c_index = summary_survival(
            model, test_loader, args.n_classes, VAE
        )
