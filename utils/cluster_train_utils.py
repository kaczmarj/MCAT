...
def train_loop_survival_cluster(
    epoch,
    model,
    loader,
    optimizer,
    ...
    for batch_idx, (
        ...
        data_WSI, cluster_id, data_omic, meta = (
            data_WSI.to(device),
            cluster_id,
            data_omic.to(device),
            meta.to(device),
        ...
        hazards, S, Y_hat, _, _ = model(
            x_path=data_WSI, cluster_id=cluster_id, x_omic=data_omic, meta=meta
        ...
        if (batch_idx + 1) % 100 == 0:
            print(
                "batch {}, loss: {:.4f}, label: {}, event_time: {:.4f}, risk: {:.4f}, bag_size: {}".format(
                    batch_idx,
                    loss_value + loss_reg,
                    label.item(),
                    float(event_time),
...
def validate_survival_cluster(
    ...
    if epoch == 10:
        torch.save(
            model.state_dict(),
            os.path.join(results_dir, "s_%d_mid_checkpoint.pt" % cur),
        )
    print(
        "\nVal Set, val_loss_surv: {:.4f}, val_loss: {:.4f}, val c-index: {:.4f}".format(
            val_loss_surv, val_loss, c_index
        )
    )
...
def summary_survival_cluster(model, loader, n_classes, VAE):
    ...
    for batch_idx, (
        ...
        with torch.no_grad():
            hazards, survival, Y_hat, _, _ = model(
                x_path=data_WSI, cluster_id=cluster_id, x_omic=data_omic, meta=meta
            )  # return hazards, S, Y_hat, A_raw, results_dict
