...
class Generic_WSI_Survival_Dataset(Dataset):
    def __init__(
        ...
        if (
            "IDC" in slide_data["oncotree_code"]
    ...
    def get_split_from_df(
        self, all_splits: dict, split_key: str = "train", scaler=None
    ):
    ...
    def return_splits(self, from_id: bool = True, csv_path: str = None):
        if from_id:
            ...
            test_split = (
                None  # self.get_split_from_df(all_splits=all_splits, split_key='test')
            )
        ...
        if not self.use_h5:
            if self.data_dir:
                if self.mode == "path":
                    ...
                    path_features = torch.cat(path_features, dim=0)
                    omic1 = torch.tensor(
                        self.genomic_features[self.omic_names[0]].iloc[idx]
                    )
                    omic2 = torch.tensor(
                        self.genomic_features[self.omic_names[1]].iloc[idx]
                    )
                    omic3 = torch.tensor(
                        self.genomic_features[self.omic_names[2]].iloc[idx]
                    )
                    omic4 = torch.tensor(
                        self.genomic_features[self.omic_names[3]].iloc[idx]
                    )
                    omic5 = torch.tensor(
                        self.genomic_features[self.omic_names[4]].iloc[idx]
                    )
                    omic6 = torch.tensor(
                        self.genomic_features[self.omic_names[5]].iloc[idx]
                    )
                    return (
                        path_features,
                        omic1,
...
class Generic_Split(Generic_MIL_Survival_Dataset):
    def __init__(
        ...
        print(
            "Skipping the loading of fast_cluster_ids.pkl because that is only used when clustering"
        )
