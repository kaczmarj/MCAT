...
class Generic_WSI_Classification_Dataset(Dataset):
    def __init__(
        ...
        slide_data = self.df_prep(
            slide_data, self.label_dict, ignore, self.label_col, multi_site
        )
    ...
    def sample_held_out(self, test_num=(40, 40)):
        ...
        for c in range(len(test_num)):
            test_ids.extend(
                np.random.choice(cls_ids[c], test_num[c], replace=False)
            )  # validation ids
...
class Generic_WSI_Inference_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        full_path = os.path.join(self.data_dir, slide_file)
        features = torch.load(full_path)
        return features
